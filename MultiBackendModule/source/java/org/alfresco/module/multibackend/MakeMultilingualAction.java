...
public class MakeMultilingualAction extends ActionExecuterAbstractBase {
	...
	protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
		paramList.add(new ParameterDefinitionImpl(PARAM_AUTHOR, DataTypeDefinition.TEXT, true,
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		if (logger.isDebugEnabled()) {
			logger.debug("Action upon NodeRef in MakeMultilingualAction:" + actionedUponNodeRef);
			logger.debug("Locale: " + (String) action.getParameterValue(PARAM_LANGUAGE));
			logger.debug("Author: " + (String) action.getParameterValue(PARAM_AUTHOR));
		}
		...
		String nodeAuthor = (String) nodeService.getProperty(actionedUponNodeRef,
		...
		NodeRef mlContainer = multilingualContentService
				.getTranslationContainer(actionedUponNodeRef);
